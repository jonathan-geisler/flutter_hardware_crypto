...
interface HardwareCryptoApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: HardwareCryptoApi?) {
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.hardware_crypto.HardwareCryptoApi.isSupported", codec)
        if (api != null) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.hardware_crypto.HardwareCryptoApi.deleteKeyPair", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val aliasArg = args[0] as String
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.hardware_crypto.HardwareCryptoApi.sign", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val aliasArg = args[0] as String
            val dataArg = args[1] as ByteArray
            api.sign(aliasArg, dataArg) { result: Result<ByteArray> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
